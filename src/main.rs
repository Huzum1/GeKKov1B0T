...
async fn run_merge_task(
    ...
    loop {
        ...
        if merge_running.load(Ordering::Relaxed) {
            info!("Another merge is already running, skipping scheduled merge round");
            sleep(interval).await;
            continue;
        }

        merge_running.store(true, Ordering::Relaxed);
        ...
        {
            Ok(count) => {
                info!("Scheduled merge finished: {count} markets");
            }
            Err(e) => {
                warn!(error = %e, "Failed to run scheduled merge");
            }
        }
        merge_running.store(false, Ordering::Relaxed);
        sleep(interval).await;
    }
}
...
async fn main() -> Result<()> {
    ...
    // Validate private key format and build the signer once. This single,
    // chain-id-applied signer is reused by TradingExecutor for every order
    // build/sign call, instead of re-parsing the private key hex on each
    // trade and on every 1s tick of the re-hedge loop.
    ...
    // Guards against the scheduled merge task and the wind-down merge running concurrently
    let merge_running = Arc::new(AtomicBool::new(false));

    // Minimum interval between two arbitrage trades on the SAME market.
    // Keyed per market_id (DashMap) instead of a single global lock, so an
    // attempt on one market never waits behind/blocks an attempt on a
    // different one.
    const MIN_TRADE_INTERVAL: Duration = Duration::from_secs(3);
    let trade_cooldowns: Arc<DashMap<B256, Instant>> = Arc::new(DashMap::new());
    ...
    if merge_interval > 0 {
        if let Some(proxy) = config.proxy_address {
            ...
            let merge_running_task = merge_running.clone();
    ...
    loop {
        ...
        loop {
            ...
            if config.wind_down_before_window_end_seconds > 0 && !wind_down_done {
                let now = Utc::now();
                let seconds_until_end = (window_end - now).num_seconds();
                let threshold_seconds = config.wind_down_before_window_end_seconds as i64;
                if seconds_until_end <= threshold_seconds {
                    ...
                    let wind_down_flag = wind_down_in_progress.clone();
                    let merge_running_wd = merge_running.clone();
                    ...
                    tokio::spawn(async move {
                        ...
                        merge_running_wd.store(true, Ordering::Relaxed);
                        ...
                        merge_running_wd.store(false, Ordering::Relaxed);
                                ...
                                if let Some(total_price) = total_ask_price {
                                    if total_price <= execution_threshold {
                                        if let Some(opp) = _detector.check_arbitrage(
                                            ...
                                            if config.stop_arbitrage_before_end_seconds > 0 {
                                                if let Some(market_info) = market_map.get(&pair.market_id) {
                                                    ...
                                                    let threshold_seconds =
                                                        config.stop_arbitrage_before_end_seconds
                                                            as i64;
                                            ...
                                            use rust_decimal::Decimal;
                                            let max_order_size =
                                                Decimal::try_from(config.max_order_size_usdc)
                                                    .unwrap_or(dec!(100.0));
                                            ...
                                            // Check trade interval: min 3s between trades on
                                            // the SAME market (per-market DashMap, no global lock).
                                            {
                                                let now = Instant::now();
                                                let mut skip_elapsed: Option<f32> = None;
                                                trade_cooldowns
                                                    .entry(opp.market_id)
                                                    .and_modify(|last| {
                                                        let elapsed = now.saturating_duration_since(*last);
                                                        if elapsed < MIN_TRADE_INTERVAL {
                                                            skip_elapsed = Some(elapsed.as_secs_f32());
                                                        } else {
                                                            *last = now;
                                                        }
                                                    })
                                                    .or_insert(now);
                                                if let Some(elapsed) = skip_elapsed {
                                                    debug!(
