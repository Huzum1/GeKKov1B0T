...
pub struct TradingExecutor {
    ...
    /// Cached, chain-id-applied signer. Built once at startup (see `main.rs`)
    /// from an already-validated private key, instead of re-parsing the hex
    /// key on every order build/sign call (hot path: arbitrage execution and
    /// the 1s-poll re-hedge loop).
...
impl TradingExecutor {
    ...
    /// All arbitrage order types submit both legs together (none stay gated).
    /// FOK/FAK submit as two independent concurrent requests (terminal IOC,
    /// reconciled by re-hedge/unwind — see
    /// `execute_arbitrage_pair_with_max_order_size`). GTD/GTC submit as a
    /// single batched `POST /orders` request and are left resting with no
    /// auto-cancel (GTD clears via its expiry, GTC via cancel_all at window
    /// switch).
    ...
    pub async fn execute_arbitrage_pair_with_max_order_size(
        ...
        // All four order types fire both legs together (no order type stays
        // gated in practice - see `is_concurrent_order_type`). FOK/FAK submit
        // as two independent concurrent requests (terminal IOC, reconciled by
        // re-hedge/unwind). GTD/GTC submit as a single batched POST /orders
        // request and are left resting (no auto-cancel) until they fill or the
        // GTD expiry elapses.
        ...
        let (signed_yes_result, signed_no_result) = tokio::join!(
            self.client.sign(&self.signer, yes_order),
            self.client.sign(&self.signer, no_order)
        );
        ...
        let (yes_result, no_result) = if Self::is_terminal_ioc_order_type(&self.arbitrage_order_type) {
            // ===== FOK/FAK: two concurrent single-order requests =====
            // Fire both legs at once via two independent POST /order calls:
            // wall-clock ~= a single round-trip, removing the second-leg delay
            // that let prices move away under sequential gating.
            //
            // Concurrency does NOT eliminate one-sided fills; FOK/FAK residuals
            // are terminal and reconciled by re-hedge/unwind below. Batched
            // submission (POST /orders) is intentionally NOT used here: the
            // "no orders found to match" FAK/FOK rejection surfaces as an
            // HTTP-level error on the single-order endpoint (see
            // `ioc_no_match_zero_fill_response`), and it is not confirmed
            // whether the batch endpoint reports that per-order or fails the
            // whole batch — the latter would hide a genuine fill on the other
            // leg. Revisit once confirmed against the live API.
            debug!(
            ...
            // ===== GTD/GTC: single batched request (POST /orders) =====
            // Both legs are submitted in exactly one HTTP request instead of
            // two independent ones. This removes the scenario where one leg's
            // HTTP submission fails (network blip) while the other's
            // independently succeeds and fills — either both legs are
            // evaluated by the matching engine from the same request, or
            // neither is (a single network/auth failure covers both). GTD/GTC
            // orders never hit the FOK/FAK "no match" HTTP-error quirk (they
            // rest instead of being killed), so batching here carries none of
            // the ambiguity that holds back the FOK/FAK path above.
            debug!(
                "Batch send ({}); single POST /orders request | {}",
                self.arbitrage_order_type,
                &pair_id[..8]
            );
            match self.client.post_orders(vec![signed_yes, signed_no]).await {
                Ok(responses) => match Self::unpack_batch_pair(responses) {
                    Ok(pair) => pair,
                    Err(e) => {
                        return Self::log_send_error(
                            &pair_id,
    ...
    /// Splits a `post_orders` batch response into (yes, no), preserving the
    /// index-aligned order the SDK guarantees (`orders[0]` -> `responses[0]`,
    /// `orders[1]` -> `responses[1]`; the SDK's own `post_orders` correlates
    /// entries by index internally for trade-hash resolution). Errors
    /// defensively if the batch didn't come back with exactly the 2 entries
    /// submitted, rather than guessing which element is which.
    fn unpack_batch_pair(
        responses: Vec<PostOrderResponse>,
    ) -> std::result::Result<(PostOrderResponse, PostOrderResponse), String> {
        if responses.len() != 2 {
            return Err(format!(
                "post_orders returned {} entries, expected 2",
                responses.len()
            ));
        }
        let mut responses = responses.into_iter();
        let yes = responses.next().expect("length checked above");
        let no = responses.next().expect("length checked above");
        Ok((yes, no))
    }
    ...
    async fn buy_at_price(&self, token: U256, price: Decimal, size: Decimal) -> Decimal {
        ...
        let signed = match self.client.sign(&self.signer, order).await {
            Ok(s) => s,
            Err(e) => {
...
mod tests {
    ...
    #[test]
    fn unpack_batch_pair_preserves_yes_no_order() {
        let yes = PostOrderResponse::builder()
            ...
            .success(true)
            .build();
        let no = PostOrderResponse::builder()
        ...
        let (unpacked_yes, unpacked_no) =
            TradingExecutor::unpack_batch_pair(vec![yes, no]).expect("2 entries should unpack");

        assert_eq!(unpacked_yes.order_id, "yes-order");
        assert_eq!(unpacked_no.order_id, "no-order");
    }

    #[test]
    fn unpack_batch_pair_rejects_unexpected_length() {
        let one = PostOrderResponse::builder()
            .order_id("only-order".to_string())
            .making_amount(dec!(0))
            .taking_amount(dec!(0))
            .status(OrderStatusType::Unmatched)
            .success(true)
        ...
        assert!(TradingExecutor::unpack_batch_pair(vec![]).is_err());
        assert!(TradingExecutor::unpack_batch_pair(vec![one.clone()]).is_err());
        assert!(TradingExecutor::unpack_batch_pair(vec![one.clone(), one.clone(), one]).is_err());
    }
