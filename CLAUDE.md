...
This repository is `polypulse`, a Rust 2021 Polymarket crypto 5-minute Up/Down arbitrage bot. It connects to live markets, can place real orders, can merge/redeem positions on Polygon, and includes an optional TUI. Treat every execution-path change as potentially money-impacting.
...
- `src/main.rs` starts the bot, initializes config, market discovery, monitoring, trading, merge, wind-down, and TUI.
...
- The bot trades immediately on startup using the static risk limits configured in `.env`; there is no runtime pause/resume mechanism.
...
- Add or update focused unit tests for trading math, risk limits, gas calculations, and reconciliation behavior.
